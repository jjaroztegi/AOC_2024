...
bool is_int(const long long &num_1, const long long &num_2,
...
vector<long long> solve_xy(int a1, int a2, int b1, int b2, long long c1,
                           long long c2) {
...
long long calc_tokens(const vector<vector<int>> &input_mat) {
    ...
    for (size_t i = 0; i < input_mat.size(); i += 3) {
        ...
        long long c1 = input_mat[i + 2][0] + 10000000000000,
                  c2 = input_mat[i + 2][1] + 10000000000000;
...
int main() {
    ...
    cout << total_tokens
         << " is the fewest tokens you would have to spend to win all possible "
            "prizes."
         << endl;
