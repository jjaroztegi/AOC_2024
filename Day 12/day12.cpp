...
bool check_same(int i, int j, const vector<vector<string>> &input_matrix,
                const string &plant) {
    int rows = input_matrix.size();
    int cols = input_matrix[0].size();
    return i >= 0 && i < rows && j >= 0 && j < cols &&
           input_matrix[i][j] == plant;
}

int get_corners(int i, int j, const vector<vector<string>> &input_matrix,
    ...
    // Each N, S, E, W, NW, NE, SW, SE checks whether the plant at the current
    // position has the same plant adjacent 8 positions (NW, NE, SW, SE are the
    // diagonals).
...
void get_region(const vector<vector<string>> &input_matrix,
                vector<vector<bool>> &visited, int start_x, int start_y,
                Region &region) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (new_x < 0 || new_x >= rows || new_y < 0 || new_y >= cols) {
            region.perimeter++;
                   ...
                   !visited[new_x][new_y] &&
                   input_matrix[new_x][new_y] ==
                       input_matrix[start_x][start_y]) {
            region.area++;
            get_region(input_matrix, visited, new_x, new_y, region);
        } else if (input_matrix[new_x][new_y] !=
                   input_matrix[start_x][start_y]) {
            region.perimeter++;
        }
    }
