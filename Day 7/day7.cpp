...
bool solve_equation(const vector<long long> &arr, int index, long long current_result, long long correct_result, vector<int> &operation_history)
{
    if (index == arr.size())
    {
    ...
    for (int operation : {0, 1, 2})
    {
        ...
        // Recurse
        if (solve_equation(arr, index + 1, new_result, correct_result, operation_history))
        {
            return true;
        }
        // Backtrack
