...
void print_values(const vector<long long> &arrangement, int num_blinks,
                  ofstream &output) {
    if (num_blinks == 0) {
...
int main() {
    ...
    for (size_t i = 0; i < 26; i++) {
        ...
        // cout << "Step " << i << " took " << duration_cast<milliseconds>(end -
        // start).count() << " ms." << endl;
    }
    output.close();

    cout << "You will have " << arrangement.size()
         << " stones after blinking 25 times." << endl;

    auto end = high_resolution_clock::now();
    cout << "Manual method took "
