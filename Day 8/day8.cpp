...
vector<vector<string>> draw_antinodes(const vector<vector<string>> &arr, const vector<int> &location, const string antenna, int &count, vector<vector<bool>> &counted)
{
...
vector<vector<string>> create_antinodes(const vector<vector<string>> &arr, int &count)
{
    vector<vector<string>> changed_arr = arr;
    vector<string> unique_antennas;

    vector<vector<bool>> counted(arr.size(), vector<bool>(arr[0].size(), false));
    ...
    for (size_t k = 0; k < unique_antennas.size(); k++)
    {
        ...
        for (size_t i = 0; i < arr.size(); i++)
        {
            for (size_t j = 0; j < arr[i].size(); j++)
            {
                if (arr[i][j] == antenna)
                {
                    vector<int> temp_vector = {static_cast<int>(i), static_cast<int>(j)};
        ...
        for (size_t i = 0; i < antenna_coords.size() - 1; i++)
        {
            for (size_t j = i + 1; j < antenna_coords.size(); j++)
            {
                ...
                changed_arr = draw_antinodes(changed_arr, location, antenna, count, counted);
            }
        }
    }
    return changed_arr;
}
vector<vector<string>> create_full_antinodes(const vector<vector<string>> &arr, int &count_2)
{
    vector<vector<string>> changed_arr = arr;
    vector<string> unique_antennas;
    ...
    for (size_t k = 0; k < unique_antennas.size(); k++)
    {
        ...
        for (size_t i = 0; i < antenna_coords.size() - 1; i++)
        {
            for (size_t j = i + 1; j < antenna_coords.size(); j++)
            {
                ...
                vector<int> location = calc_antinodes(coordinates);
                changed_arr = draw_antinodes(changed_arr, location, antenna, count_2, counted);
                ...
                while (!out_of_bounds)
                {
                    ...
                    vector<int> next_location = calc_antinodes(next_coordinates);

                    if (next_location[0] < 0 || next_location[1] < 0 ||
                        next_location[2] < 0 || next_location[3] < 0 ||
                        next_location[0] >= arr.size() || next_location[1] >= arr[0].size() ||
                        next_location[2] >= arr.size() || next_location[3] >= arr[0].size())
                    {
                        out_of_bounds = true;
                        break;
                    }
                    changed_arr = draw_antinodes(changed_arr, next_location, antenna, count_2, counted);
...
int main()
{
    ...
    int count_2 = 0;
    vector<vector<string>> full_antinode_arr = create_full_antinodes(arr, count_2);
    ...
    cout << "Part 1: There are " << count << " unique locations within the bounds of the map that contain an antinode." << endl;
    cout << "Part 2: There are " << count_2 << " unique locations within the bounds of the map that contain an antinode." << endl;
